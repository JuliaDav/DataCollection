...
# обновляем инфо в базе
            count = vacs.count_documents({'link':main_link_sj + vac_link})
            if count == 0:
                vacs.insert_one(vac_data)
            else:
                vacs.update_one(vac_data, {'$set': vac_data})
...
# делим зп на минимальную и максимальную
            ...
            vac_data['from'] = 'hh'
# обновляем инфо в базе
            count = vacs.count_documents({'link': vac_link})
            if count == 0:
                vacs.insert_one(vac_data)
            else:
                vacs.update_one(vac_data, {'$set': vac_data})
...
salary_sort = vacs.find({'$or':[{'min':{'$gte': 250000}},{'max':{'$gte': 250000}}]})
